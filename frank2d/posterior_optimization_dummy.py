...
class GaussianModel:
    ...
    def emcee_optimizer(self):
        import emcee
        from multiprocessing import Pool
        print("Using emcee for optimization")
        nwalkers = 100
        ndim = 2
        burn = 100
        steps = 1000
        ...
        # --- Definir log_prob como método de instancia (picklable) ---
        ...
        # --- Crear sampler con paralelización ---
        with Pool() as pool:
            ...
            try:
                sampler.run_mcmc(p0, steps, progress=True, store=True)
            except KeyboardInterrupt:
                print("✋ Muestreo interrumpido. Guardando lo corrido en memoria...")
            finally:

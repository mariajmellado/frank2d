...
class PostProcess(object):
    ...
    def separate_data(self):
        ...
        index_w  = cp.flatnonzero(r)
        index_uw = cp.flatnonzero(~r)  
    ...
    def build_vis_model(self, kernel, kernel_params, vis_sol_weighted):
        ...
        if len(index_uw) == 0:
            self.show.warning("No unweighted data found. Returning weighted visibility model only.")
            V_full = cp.zeros((self._Nx, self._Ny), dtype="c16")

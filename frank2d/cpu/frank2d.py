...
class Frank2D(object):
    ...
    def check_bounds(self, u, v):
            ...
            Qmax_data = np.sqrt(u**2 + v**2).max()
            Qmin_data = np.sqrt(u**2 + v**2).min()
    ...
    def set_fit_method(  self, method_name = 'bicgstab',
        ...
        precond_type : str
            ...
            - jacobi: Jacobi preconditioner, which uses the inverse of the diagonal of the matrix.
            - ilu: Incomplete LU preconditioner, which approximates the LU decomposition of the matrix.
        """
    ...
    def set_MAPEstimator(self, N_opt = 50, verbose = False):
        """
        Setter of the MAPEstimator object.
        ...
        self._MAPEstimator = MAPEstimator(self._Rmax, N_opt = N_opt, verbose = verbose)
    ...
    def process_vis(self, data, hermitian = True, verbose = False):
        ...
        self.process_gridded_vis( verbose = verbose)
    ...
    def search_MAP(self, data = None,
        ...
        if not self._set_MAP:
            if data is None:
                self.show.warning("Using existing visibility data...")
                if not self._set_gridded_data:
            ...
            self._MAPEstimator.optimize(data, initial_guess)
            self._MAP = self._MAPEstimator.MAP
    ...
    @property
    def xy_points(self):
        """ Collocation points in x and y in arcseconds."""
        return self.FT.xy_points * rad_to_arcsec

    @property
    def uv_points(self):
        """ Collocation points in u and v in lambda."""
